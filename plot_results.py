...
def build_input_csv_files(csv_file, no_averaging=True, time_horizons=None):
    ...
    If no_averaging is True, each file is generated by inserting
...
def extract_column_data(csv_files, y_col_num, filter_by="graph_type", filter_values=None):
    """
    Extract x, average y, and std data from one or more CSV files.

    The selected y_col_num is treated as the average column. The standard
    ...
    print(
        f"Found {len(x_vals)} total data points for average column {y_col_num} "
        f"and std column {y_col_num + 1} "
        f"with {filter_by} filter {sorted(filter_values)}."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_false",
        help=(
            "Disable no-averaging mode and load the CSV file specified by --csv directly."
        ),
    )
